# Why You Shouldn't use OnAfterRender in Blazor

I'll start this article with a qualification to that statement.

> If your `OnAfterRender{Async}` code isn't doing JS interop stuff, it's in the wrong place!

I'll explain why.

## The Synchronisation Context

Blazor uses a Synchronisation Context to manage the UI processes.  The SC a virtual thread that manages the UI process and guarantees a Task based single thread of execution.  I'll use **SC** throughout the rest of this article: *Synchronisation Context* is too long to repetatively type! 
...
`TaskYieldAsync` yields to `OnInitializedAsync` which yields to the internal `ComponentBase` method `RunInitAndSetParametersAsync`. This in turn yields to the SC, letting the renderer render the component.  

```text
Component Rendered e45633be-c5a1-451b-99a2-ce64f128a65c.
```

At this point the renderer queues a `OnAfterRender` onto the SC.  The queue has two tasks:

1. The continuation from `OnInitializedAsync`.
2. `OnAfterRender` for the component.
 
As the `OnInitializedAsync` is effectively complete, it's prioritized and runs next. 
...
At this point a second `OnAfterRender` is queued onto the SC. 
...
But now a change.  The `OnAfterRender` associated with the first render happens ahead of the `OnInitializedAsync` continuation because it's timer hasn't expired.
...
There's plenty of voodoo, myth and many Mad Max ideas associated with `OnAfterRender`.  The simple answer to those is: get your normal lifecycle logic correct.

There are other, more nuanced situations, where an experienced programmer may be tempted.

Don't be. You're coding logic into a system where timing affects the order in which the lifecycle methods execute.
